				                	...
				                	<c:forEach items="${satellite_list_attribute }" var="satelliteItem">
										<tr>
											...
											<td>
												...
												<c:if test="${satelliteItem.dataLancio == null || satelliteItem.dataLancio.after(now_attr) }">
												<form:form action="${pageContext.request.contextPath}/satellite/lancio" method="post">
													<input type="hidden" value="${satelliteItem.id }" name="lancio_attr">
													<button type="submit" name="submit" value="submit" id="submit" class="btn btn-warning btn-sm">Lancio</button>
												</form:form>	
												</c:if>
												
												<c:if test="${satelliteItem.dataRientro == null || satelliteItem.dataRientro.after(now_attr)}">
												<form:form action="${pageContext.request.contextPath}/satellite/rientro" method="post">
													<input type="hidden" value="${satelliteItem.id }" name="rientro_attr">
													<button type="submit" name="submit" value="submit" id="submit" class="btn btn-info btn-sm">Rientro</button>
												</form:form>	
												</c:if>
											</td>
										</tr>
